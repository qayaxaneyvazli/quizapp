...
const String _duelnotificationsEnabledKey = 'duel_notifications_enabled';

// Provider to check if notifications are enabled
final duelnotificationsEnabledProvider = StateNotifierProvider<NotificationsNotifier, bool>((ref) {
...
class NotificationsNotifier extends StateNotifier<bool> {
  ...
  Future<void> _loadPreference() async {
    try {
      ...
      final enabled = prefs.getBool(_duelnotificationsEnabledKey) ?? true;
      state = enabled;
    } catch (e) {
  ...
  Future<void> _savePreference(bool value) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      await prefs.setBool(_duelnotificationsEnabledKey, value);
    } catch (e) {
  ...
  Future<void> setDuelNotificationsEnabled(bool enabled) async {
    if (state != enabled) {
      state = enabled;
      await _savePreference(state);
